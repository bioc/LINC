...
my_lincRNAs <- c("ENSG00000224153", "ENSG00000197813", "ENSG00000179136",
                 "ENSG00000259439", "ENSG00000267462")
res <- justlinc(GTEX_LIVER_CRUDE, targetGenes = my_lincRNAs)
```

The optional argument *targetGenes* can be either a gene *'biotype'* (the default value is *'lincRNA'*), a single candidate gene or a vector of genes. For multiple candidate genes it is assumed that they have a connection, for instance comparable co-expression pattern. As distance measure of two lincRNAs the Czekanovski dice distance [6] is used.
...
```{r fig.width = 17, fig.height = 9.5, warning = FALSE, eval = TRUE}
suppressWarnings(suppressMessages(require(LINC))); data(BRAIN_EXPR)
...
## 2.1 Advanced 2: single query or cluster analysis

Starting from the correlation matrix, the *LINCmatrix* object, there are two functions to compute either (I) a cluster with *clusterlinc()* or (II) an analysis for a single gene. For a single query gene they function *singlelinc()* comes with many options in terms of co-expression selection. One can select for absolute values, negative correlations and a maximum for co-expressed genes. Here, the query is 'MEG3 - maternally expressed 3' (its Entrez gene id is '55384'). In contrast to *clusterlinc()*, *singlelinc()* will directly call a gene annotation resource for the co-expressed genes.

```{r fig.width = 21, fig.height = 10, warning = FALSE, message = FALSE, eval = TRUE}
meg3 <- singlelinc(crbl_matrix, query = "55384", threshold = 0.00005, ont = 'BP', verbose = FALSE)
plotlinc(meg3)
```

The *clusterlinc()* functions perfroms a correlation test (Spearman's rho by default) and clusters the candidate lincRNAs based on their interaction partners or their correlations. Enriched biological terms from gene annotation resources like *Reactome PA* and *Gene Ontology (GO)* for the cluster (multiple candidate lincRNA genes) can be identified applying *getbio()* which will call supported functions from the *clusterProfiler* package. [7 - 9] This analysis will reveal which functions, pathways or compartments are associated with the lincRNA-co-expressed genes.  

```{r fig.width = 13.5, fig.height = 11.5, warning = FALSE, eval = TRUE}
# These lines of code will create a 'LINCmatrix' instance,
# cluster the lincRNAs, derive enriched terms and plot the heatmap

ctx_matrix <- linc(cortex, codingGenes = pcgenes_ctx, verbose = FALSE)
ctx_terms <- getbio(clusterlinc(ctx_matrix, verbose = FALSE),
                    translate = 'none', annotateFrom = 'enrichPathway')
plotlinc(ctx_terms)
```
This relationship is represented by the dendrogram (marked in dark red). As distance between two lincRNAs the Czekanovski dice distance [6] is considered, a measure for the number of shared interaction partners. There are different clustering methods to choose from. *"dicedist"* is the default.
...
```{r fig.width = 9, fig.height = 7.5, fig.keep='first', warning = FALSE, message = FALSE, eval = FALSE}
