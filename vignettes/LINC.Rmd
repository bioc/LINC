...
meg3 <- singlelinc(crbl_matrix, query = "55384",
                   ...
                   threshold = 0.5, enrichFun = 'enrichPathway')
plotlinc(meg3)
```

This yields 22 genes which have a correlation of > 0.5 to the expression profile of MEG3. As for the cluster analysis 'mRNA Splicing' is again significant.
...
Removing PCs changed the prediction! While 'mRNA Splicing' is still consistent (since this observation does not depend on the main variance in the dataset) the term 'Transcription' is no longer significant.
...
res <- justlinc(GTEX_LIVER_CRUDE, targetGenes = my_lincRNAs) # 'justlinc' with queries
```
...
```{r fig.width = 13.5, fig.height = 6.5, warning = FALSE, fig.keep = 'last', eval = FALSE}
